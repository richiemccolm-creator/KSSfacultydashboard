/**
 * Hub bridge for QS Attainment Dashboard: saved-year library + Save to Hub.
...
(function () {
  ...
  function setStatus(msg, kind) {
    ...
    statusEl.className = 'qs-hub-status' + (kind ? ' qs-hub-status--' + kind : '');
  }
  ...
  function renderLibrary(rows) {
    ...
    if (!rows || !rows.length) {
      libraryEl.innerHTML =
        '<div class="qs-hub-empty">No saved sessions yet. Upload the QS workbook and save it to the hub for staff to review.</div>';
      return;
    }
  ...
  function refreshLibrary(preferYear) {
    ...
    return Svc.listSnapshots()
      .then(function (rows) {
        ...
        setStatus(
          rows.length
            ? rows.length + ' saved session' + (rows.length === 1 ? '' : 's')
            : 'No saved sessions yet.'
        );
        if (!preferYear && !currentCloudYear && rows.length && !window.__SNAPSHOT__) {
          var hero = $('uploadHero');
          if (hero && !hero.classList.contains('hidden')) {
            return Svc.getSnapshot(rows[0].school_year).then(function (full) {
  ...
  function saveToHub() {
    ...
    var session = payload.session || ($('ySession') && $('ySession').value) || '';
    if (!session) {
      ...
      if ($('ySession')) {
        var opt = Array.prototype.find.call($('ySession').options, function (o) {
          return o.value === session;
        });
        if (!opt) {
          $('ySession').insertAdjacentHTML(
            'beforeend',
            '<option value="' + session + '">' + session + '</option>'
          );
        }
        $('ySession').value = session;
      }
    ...
    Svc.saveSnapshot({
      ...
      .then(function (row) {
        ...
        setStatus(
          'Saved ' + currentCloudYear + ' — staff can open it anytime from Improvement.',
          'ok'
        );
        var banner = $('hubCloudBanner');
        if (banner) {
          banner.classList.remove('hidden');
          banner.innerHTML =
            'Saved in Faculty Hub · <b>' + escapeHtml(currentCloudYear) + '</b> · just now';
        }
        syncManagerUi();
  ...
  function deleteFromHub() {
    ...
    if (
      !window.confirm(
        'Delete the saved ' +
          currentCloudYear +
          ' attainment snapshot from the hub? This cannot be undone.'
      )
    ) {
      return;
    }
    setStatus('Deleting ' + currentCloudYear + '…');
    Svc.deleteSnapshot(currentCloudYear)
      .then(function () {
        setStatus('Deleted ' + currentCloudYear + '.', 'ok');
  ...
  function onDataLoaded() {
    if (!window.__qsLoadingFromCloud) {
      ...
      if (banner) {
        banner.classList.remove('hidden');
        banner.innerHTML =
          'Local workbook loaded — <b>not saved to the hub yet</b>. Faculty heads: use Save to Hub to keep this session.';
      }
      if (libraryEl) {
        libraryEl.querySelectorAll('.qs-hub-year.on').forEach(function (el) {
    ...
    if (canManage() && saveBtn && !window.__qsLoadingFromCloud) {
      setStatus('Data ready — save to the hub so staff can come back to this session.', 'ok');
    }
  }
